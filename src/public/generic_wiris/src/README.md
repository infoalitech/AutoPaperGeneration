...
To integrate MathType, please follow the steps below. Please, note that you may adjust the example parameters of the example below depending on your integration.
...
4. Add the following code just before closing the body tag:
   ...
   * Initializes the integration into the editable HTML element, assigning listeners to some events of the HTML element. This allows, for instance, to open a formula just by double-clicking on it. You may change **target** and **toolbar** parameters depending on the structure of your HTML editor.
   * It is expected that the **target** property contains the editable HTML element. This parameter is mandatory.
   * It is expected that the toolbar** property contains the HTML element that represents the toolbar. This parameter is optional.
...
By default, MathType stores equations and the formulas as **MathML**. However, while editing, MathType converts all MathML into `img` tags that are rendered inside the editable HTML element.
